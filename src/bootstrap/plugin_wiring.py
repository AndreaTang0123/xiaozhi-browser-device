...
集中：创建 McpServer、注册插件清单、资源池清理、音频直连。
"""
...
def bind_shared_services(container: "ServiceContainer") -> None:
    ...
    logger.debug("共享服务已创建: McpServer")
...
async def setup_plugins(
    ...
    mcp_plugin = McpPlugin(server=container.mcp_server)
